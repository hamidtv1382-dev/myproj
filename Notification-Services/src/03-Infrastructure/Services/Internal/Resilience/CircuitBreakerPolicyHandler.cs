namespace Notification_Services.src._03_Infrastructure.Services.Internal.Resilience
{
    public class CircuitBreakerPolicyHandler
    {
        ...
        public async Task<T> ExecuteAsync<T>(Func<Task<T>> action)
        {
            if (IsOpen())
            {
                throw new InvalidOperationException("Circuit breaker is OPEN. Calls are blocked.");
            }
